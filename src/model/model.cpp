...
void Model::setCentre(ModelData *data) {
  if (data->getCoordinates().empty()) return;
...
void Model::affine(Strategy::SelectionStrategy selectionStrategy,
                   Strategy::TypeCoordinate typeCoordinate, ModelData *data,
                   double point) {
  if (selectionStrategy == Strategy::SelectionStrategy::ZOOM) {
    affineTransformations.setStrategy(zoomObj);
  } else if (selectionStrategy == Strategy::SelectionStrategy::MOVE) {
    affineTransformations.setStrategy(moveObj);
  } else if (selectionStrategy == Strategy::SelectionStrategy::ROTATE) {
    affineTransformations.setStrategy(rotateObj);
  }

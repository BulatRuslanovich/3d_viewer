...
GLWidget::GLWidget(QWidget *parent, s21::Controller *c)
    : QOpenGLWidget(parent), controller(c) {
  setDefault();
}
...
void GLWidget::drawVertexes() {
  ...
  if (vertexesType != VertexesType::NONE) {
    ...
    glDrawElements(GL_POINTS, (int)controller->getData().getPolygons().size(),
                   GL_UNSIGNED_INT, controller->getData().getPolygons().data());
...
void GLWidget::drawLines() {
  ...
  glVertexPointer(3, GL_DOUBLE, 0,
                  controller->getData().getCoordinates().data());
  glDrawElements(GL_LINES, (int)controller->getData().getPolygons().size(),
                 GL_UNSIGNED_INT, controller->getData().getPolygons().data());
  glDisableClientState(GL_VERTEX_ARRAY);
}
...
void GLWidget::mouseMoveEvent(QMouseEvent *mouseEvent) {
  ...
  controller->affine(Strategy::SelectionStrategy::ROTATE,
                     Strategy::TypeCoordinate::Y, &controller->getData(), yRot);
  sumRotY += yRot;
  update();
}
